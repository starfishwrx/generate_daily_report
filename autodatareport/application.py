...
from autodatareport.models import AppConfig, ExtraStageResult, FailureKind, PCStageResult, RunContext, RunOptions, SourceError
from autodatareport.pipeline import SourceTask, run_source_tasks
from autodatareport.publishing import PublishCallbacks, PublishCoordinator
...
_FONT_CONFIGURED = False
CHART_RENDERER_SCHEMA = "chart-renderer-v2"
PAYMENT_TABLE_RENDERER_SCHEMA = "payment-table-renderer-v2"
...
class PublishUncertainError(ReportError):
    """The remote write may have succeeded and requires a human decision."""
...
def prepare_template_directory(template_dir: Path, template_name: str, default_content: str) -> Path:
    ...
    if not fallback_path.exists():
        atomic_write_text(fallback_path, default_content)
    logging.warning(
...
def preflight_870_auth(
    ...
    try:
        ...
        payload = fetch_json(session, base_url, params, timeout, method, hosts_map=hosts_map_870)
        metrics = current_metrics()
        if metrics is not None:
            metrics.increment("requests")
            metrics.increment("requests_870")
    ...
    return {
        "ok": True,
        "message": f"870登录态可用: {label}",
        "reusable_responses": {_870_request_fingerprint(base_url, params, method): payload},
    }
...
def run_full_auth_preflight(  # noqa: C901 - legacy compatibility preflight
    config: Dict[str, Any],
    args: argparse.Namespace,
    ...
    if needs_extension_preflight:
        if not extra_auth_file.exists():
            raise SourceError("extra", FailureKind.CONFIG_MISSING, f"扩展登录态预检失败: 认证文件不存在：{extra_auth_file}")
    ...
    reusable_870 = dict((preflights.get("870") or {}).get("reusable_responses") or {})
    if not needs_extension_preflight:
        return {"__870_responses__": reusable_870}
    ...
    extra_auth["__870_responses__"] = reusable_870
    return extra_auth


async def run_full_auth_preflight_async(  # noqa: C901 - platform validation matrix
    config: Dict[str, Any],
    args: argparse.Namespace,
    extra_metrics_cfg: Dict[str, Any],
    extra_auth_file: Path,
) -> Tuple[Dict[str, Any], ExtraMetricsService | None, PCWebMetricsService | None]:
    """Run preflight in the collection loop so source clients remain reusable."""

    report_date_for_check = resolve_report_date(args.date)
    pc_web_cfg = config.get("pc_web_metrics") or {}
    if not isinstance(pc_web_cfg, dict):
        raise ReportError("Config field pc_web_metrics must be a mapping when provided.")
    needs_extension = bool(args.with_extra_metrics or extra_metrics_cfg.get("enabled") or pc_web_cfg.get("enabled"))
    extra_auth: Dict[str, Any] = {}
    fenxi_diag: Dict[str, Any] = {"usable": True, "message": "未启用扩展数据"}
    extra_service: ExtraMetricsService | None = None
    pc_service: PCWebMetricsService | None = None
    if needs_extension:
        if not extra_auth_file.exists():
            raise SourceError("extra", FailureKind.CONFIG_MISSING, f"扩展登录态预检失败: 认证文件不存在：{extra_auth_file}")
        extra_auth = load_extra_auth(extra_auth_file)
        fenxi_diag = diagnose_fenxi_token(
            extra_auth,
            timezone_name=str(extra_metrics_cfg.get("timezone", "Asia/Shanghai")),
            warn_threshold_hours=6.0,
        )
        hosts_yaml_path = (
            args.hosts_yaml_path if args.hosts_yaml_path is not None else str(extra_metrics_cfg.get("hosts_yaml_path", ""))
        )
        query_proxy_url = (
            args.query_proxy_url if args.query_proxy_url is not None else str(extra_metrics_cfg.get("query_proxy_url", ""))
        )
        extra_service = ExtraMetricsService(
            ExtraSettings(
                timezone=str(extra_metrics_cfg.get("timezone", "Asia/Shanghai")),
                request_timeout=int(extra_metrics_cfg.get("request_timeout", 30)),
                query_proxy_url=str(query_proxy_url).strip(),
        ...
        extra_service.enable_client_reuse()
        if bool(pc_web_cfg.get("enabled")):
            pc_service = create_pc_web_service(config, args, extra_metrics_cfg)
            pc_service.enable_client_reuse()

    try:
        preflights = await _run_remote_preflights(
            config=config,
            args=args,
            report_date_for_check=report_date_for_check,
            extra_service=extra_service,
        ...
        if "870" in preflights:
            ...
            logging.info("870: %s", auth_870.get("message", ""))
            if not is_preflight_ok(auth_870.get("ok")):
                raise SourceError("870", FailureKind.LOGIN_REQUIRED, str(auth_870.get("message") or "870登录态预检失败"))
        reusable_870 = dict((preflights.get("870") or {}).get("reusable_responses") or {})
        if needs_extension:
            extra_preflight = preflights.get("extra") or {}
            fenxi_result = extra_preflight.get("fenxi") or {}
            manage_result = extra_preflight.get("505") or {}
            if not (is_preflight_ok(fenxi_result.get("ok")) and bool(fenxi_diag.get("usable"))):
                raise SourceError(
                    "fenxi",
                    FailureKind.LOGIN_REQUIRED,
                    f"分析后台登录态预检失败: {fenxi_result.get('message', fenxi_diag.get('message', ''))}",
                )
            if not is_preflight_ok(manage_result.get("ok")):
                raise SourceError("505", FailureKind.LOGIN_REQUIRED, f"505后台登录态预检失败: {manage_result.get('message', '')}")
            if pc_service is not None:
                strict_mode = bool(pc_web_cfg.get("strict", True))
                pc_preflight = preflights.get("pc_web") or {}
                if not bool(pc_preflight.get("ok")) and strict_mode:
                    raise SourceError("pc_web", FailureKind.LOGIN_REQUIRED, f"PC后台登录态预检失败: {pc_preflight.get('message', '')}")
                if bool(pc_web_cfg.get("include_member_metrics", True)):
                    member_preflight = preflights.get("pc_member") or {}
                    if not (bool(member_preflight.get("ok")) and bool(fenxi_diag.get("usable"))) and strict_mode:
                        raise SourceError(
                            "pc_member",
                            FailureKind.LOGIN_REQUIRED,
                            f"PC会员登录态预检失败: {member_preflight.get('message', '')}",
                        )
        extra_auth["__870_responses__"] = reusable_870
        return extra_auth, extra_service, pc_service
    except BaseException:
        if extra_service is not None:
            await extra_service.aclose()
        if pc_service is not None:
            await pc_service.aclose()
        raise
...
def auth_repair_enabled(config: Dict[str, Any], args: argparse.Namespace) -> bool:
    ...
    if env_value is not None:
        return bool(args.repair_auth_on_failure or env_value)
    return bool(args.repair_auth_on_failure or cfg.get("enabled", False))
...
def run_auth_repair(
    ...
    *,
    ...
    if cookie_870:
        ...
        atomic_write_yaml(Path(args.config), config)
...
def run_full_auth_preflight_with_repair(
    ...
    *,
    ...
    try:
        ...
        return extra_auth
    except (ReportError, SourceError) as exc:
        reason_text = str(exc)
        write_run_state(
            Path(args.output),
        ...
        if not repair_targets:
            logging.info("认证失败不属于登录态，跳过自动修复: %s", reason_text)
            raise
...
async def authenticate_daily_sources(
    context: RunContext,
    config: Dict[str, Any],
    args: argparse.Namespace,
    extra_metrics_cfg: Dict[str, Any],
    ...
    """Authenticate all sources and retain their run-scoped clients."""

    async def preflight() -> Tuple[Dict[str, Any], ExtraMetricsService | None, PCWebMetricsService | None]:
        return await run_full_auth_preflight_async(config, args, extra_metrics_cfg, extra_auth_file)

    auth_started = monotonic_time.perf_counter()
    emit_progress(8, "执行全平台登录态预检")
    try:
        extra_auth, extra_service, pc_service = await preflight()
    except ReportError as exc:
        reason_text = str(exc)
        write_run_state(
            Path(args.output),
            context.report_date,
            {"last_phase": "daily_report", "failure_stage": "auth_preflight", "failure_reason": reason_text},
        )
        if not auth_repair_enabled(config, args):
            raise
        repair_targets = sorted(classify_auth_failure(reason_text))
        if not repair_targets:
            raise
        emit_progress(9, "登录态失效，打开 Chrome 修复窗口")
        original_target = args.auth_repair_target
        if str(original_target or "auto").strip().lower() == "auto":
            args.auth_repair_target = select_auth_repair_target(repair_targets)
        try:
            ...
            args.auth_repair_target = original_target
        write_run_state(
            Path(args.output),
            context.report_date,
            {
                "preflight_result": "ok_after_repair",
                "last_phase": "daily_report",
                "repair_targets": repair_targets,
                "repair_attempted": True,
                "repair_result": "ok",
                "auth_repair_log": repair_result.get("log_path", ""),
            },
        )
    else:
        write_run_state(
            Path(args.output),
            context.report_date,
            {"preflight_result": "ok", "last_phase": "daily_report"},
        )
...
async def collect_daily_sources_with_auth(
    context: RunContext,
    args: argparse.Namespace,
    cookie: str,
    extra_metrics_cfg: Dict[str, Any],
    pc_web_cfg: Dict[str, Any],
    timeout: float,
    auth_resources: Tuple[Dict[str, Any], ExtraMetricsService | None, PCWebMetricsService | None],
) -> Dict[str, Any]:
    extra_auth, extra_service, pc_service = auth_resources
    context.shared["extra_auth"] = extra_auth
    context.shared["870_preflight_responses"] = dict(extra_auth.get("__870_responses__") or {})
    context.shared["extra_service"] = extra_service
    context.shared["pc_service"] = pc_service
    try:
        return await collect_daily_sources(context, args, cookie, extra_metrics_cfg, pc_web_cfg, timeout)
    finally:
        if extra_service is not None:
            await extra_service.aclose()
        if pc_service is not None:
...
async def run_daily_preflight_and_collect(
    context: RunContext,
    config: Dict[str, Any],
    args: argparse.Namespace,
    extra_metrics_cfg: Dict[str, Any],
    ...
    auth_resources = await authenticate_daily_sources(
        context, config, args, extra_metrics_cfg, extra_auth_file, pc_web_cfg
    )
    return await collect_daily_sources_with_auth(
        context, args, cookie, extra_metrics_cfg, pc_web_cfg, timeout, auth_resources
    )
...
def normalize_series_entry(  # noqa: C901 - stable V1.4 response normalization
    entry: Dict[str, Any],
    base_date: date,
...
def collect_series_for_queries(
    ...
    for query in queries:
        ...
        fingerprint = _870_request_fingerprint(base_url, params, method)
        if response_cache is not None and fingerprint in response_cache:
            payload = response_cache[fingerprint]
            metrics = current_metrics()
            if metrics is not None:
                metrics.increment("preflight_response_reuse")
        else:
...
def _870_request_fingerprint(base_url: str, params: Dict[str, Any], method: str) -> str:
    return hash_payload(["870-request-v1", str(base_url), str(method or "get").lower(), params])
...
"""
    atomic_write_text(output_path, html_doc)
    return output_path
...
def collect_870_stage(  # noqa: C901 - stable target/query compatibility matrix
    context: RunContext, args: argparse.Namespace, cookie: str
    ...
    def get_session() -> requests.Session:
        ...
        if metrics is not None:
            def record_response(response: requests.Response, *_args: Any, **_kwargs: Any) -> None:
                ...
                metrics.increment("requests_870", 1 + retry_count)
                if retry_count:
    ...
    def collect_target(key: str) -> TargetResult:
        return build_target_result(
            ...
            hosts_map=hosts_map,
            response_cache=context.shared.get("870_preflight_responses"),
        )
    ...
    for key in configured_keys:
        ...
        if config.get("generate_charts", True) and not args.no_charts:
            ...
            legacy_cache_key = f"chart:{context.report_date.isoformat()}:{key}"
            cache_key = f"chart:v2:{context.report_date.isoformat()}:{key}"
            input_hash = hash_payload([CHART_RENDERER_SCHEMA, __version__, _target_chart_input(result)])
            if isinstance(cache, ArtifactCache) and legacy_cache_key in cache.entries and cache_key not in cache.entries:
                if metrics is not None:
                    metrics.increment("cache_version_invalidations")
...
async def collect_extra_stage(
    ...
    if not isinstance(extra_auth, dict) or not extra_auth:
        ...
        extra_service = context.shared.get("extra_service")
        if not isinstance(extra_service, ExtraMetricsService):
            extra_service = ExtraMetricsService(
                ExtraSettings(
                    timezone=str(extra_metrics_cfg.get("timezone", "Asia/Shanghai")),
    ...
    if isinstance(payment_tables, dict) and payment_tables:
        ...
        cache = context.shared.get("artifact_cache")
        legacy_cache_key = f"payment_tables:{context.report_date.isoformat()}"
        cache_key = f"payment_tables:v2:{context.report_date.isoformat()}"
        input_hash = hash_payload([PAYMENT_TABLE_RENDERER_SCHEMA, __version__, payment_tables])
        metrics = current_metrics()
        if isinstance(cache, ArtifactCache) and legacy_cache_key in cache.entries and cache_key not in cache.entries:
            if metrics is not None:
                metrics.increment("cache_version_invalidations")
...
async def collect_pc_stage(
    ...
    pc_auth_key = str(pc_web_cfg.get("auth_key", "pc_web")).strip() or "pc_web"
    pc_auth = extra_auth.get(pc_auth_key) or extra_auth.get("pc_web")
    service = context.shared.get("pc_service")
    if not isinstance(service, PCWebMetricsService):
...
def calculate_daily_report_data(config: Dict[str, Any], source_values: Dict[str, Any]) -> Dict[str, Any]:
...
def render_daily_report_artifacts(
    context: RunContext,
    args: argparse.Namespace,
    date_cn: str,
    ordered_section_keys: Sequence[str],
    ...
    output_path = render_report(
        ...
        analysis_sentences=calculated["analysis_sentences"],
        extra_metrics_block=calculated["extra_metrics_block"],
    )
    logging.info("Report generated: %s", output_path)
    emit_event("artifact", "render_reports", "主日报已生成", details={"target": "main", "path": str(output_path)})
    chart_image_paths = {
        key: str(result.chart_path) for key, result in results.items() if result.chart_path is not None
    }
    ...
    if pc_target:
        pc_report_path = render_pc_report(
            ...
            date_cn=date_cn,
            target=pc_target,
            pc_notes=calculated["pc_report_notes"],
            pc_member_summary=calculated["pc_report_member_summary"],
            pc_top_games=calculated["pc_report_top_games"],
            pc_warnings=calculated["pc_report_warnings"],
        )
        logging.info("PC cloud report generated: %s", pc_report_path)
        emit_event("artifact", "render_reports", "PC日报已生成", details={"target": "pc", "path": str(pc_report_path)})
    finish_stage_timing("render_reports", started)
    return {
        "output_path": output_path,
        "pc_report_path": pc_report_path,
        "chart_image_paths": chart_image_paths,
        "pc_target": pc_target,
        "extra_payment_images": calculated["extra_payment_images"],
    }
...
class FeishuPublishJob:
    ...
    callbacks: Optional[PublishCallbacks] = None
...
def publish_feishu_jobs(
    ...
    def publish(job: FeishuPublishJob) -> Tuple[Dict[str, str], float]:
        ...
        result = publish_report_to_feishu_doc(
            ...
            on_publish_started=job.callbacks.started if job.callbacks else None,
            on_document_created=job.callbacks.remote_created if job.callbacks else None,
        )
...
def publish_daily_report_artifacts(
    ...
    pc_report_path: Optional[Path] = rendered["pc_report_path"]
    chart_image_paths: Dict[str, str] = rendered["chart_image_paths"]
    extra_payment_images: Dict[str, str] = rendered["extra_payment_images"]
    pc_target = rendered["pc_target"]
    publish_store = PublishStateStore(context.output_dir, context.report_date)
    coordinator = PublishCoordinator(publish_store, force=args.force_publish)
    urls = {"main": "", "pc": ""}

    if should_push_feishu_doc(args, config):
        emit_progress(90, "推送飞书文档")
        settings = resolve_feishu_doc_settings(args, config)
        jobs: List[FeishuPublishJob] = []
        hashes: Dict[str, str] = {}
        title_override, title_prefix = resolve_feishu_doc_title(args, config)
        main_hash = build_publish_hash(output_path, [*chart_image_paths.values(), *extra_payment_images.values()])
        completed = coordinator.completed_result("feishu_main", main_hash)
        if completed is not None:
            urls["main"] = str(completed.get("url") or "").strip()
            emit_event(
                "publish_finished",
                "publish_feishu_main",
                "主日报内容未变化，已跳过重复发送",
                details={"target": "main", "url": urls["main"], "skipped": True},
            )
        else:
            hashes["main"] = main_hash
            jobs.append(
                FeishuPublishJob(
                    target="main",
                    ...
                    callbacks=coordinator.callbacks("feishu_main", main_hash),
                )
            )
        if pc_target and pc_report_path is not None and should_push_feishu_pc_doc(config):
            pc_title_override, pc_title_prefix = resolve_feishu_pc_doc_title(config)
            pc_charts = {"pc_cloud": str(pc_target.chart_path)} if pc_target.chart_path is not None else {}
            pc_hash = build_publish_hash(pc_report_path, pc_charts.values())
            completed = coordinator.completed_result("feishu_pc", pc_hash)
            if completed is not None:
                urls["pc"] = str(completed.get("url") or "").strip()
                emit_event(
                    "publish_finished",
                    "publish_feishu_pc",
                    "PC日报内容未变化，已跳过重复发送",
                    details={"target": "pc", "url": urls["pc"], "skipped": True},
                )
            else:
                hashes["pc"] = pc_hash
                jobs.append(
                    FeishuPublishJob(
                        ...
                        chart_image_paths=pc_charts,
                        callbacks=coordinator.callbacks("feishu_pc", pc_hash),
                    )
                )
        batch = publish_feishu_jobs(settings, jobs)
        for target, (result, elapsed) in batch.results.items():
            state_key = f"feishu_{target}"
            url = str(result.get("url") or "").strip()
            coordinator.completed(state_key, hashes[target], result)
            urls[target] = url
        ...
        for target, error in batch.errors.items():
            state_key = f"feishu_{target}"
            payload_hash = hashes[target]
            entry = publish_store.entry(state_key, payload_hash)
            coordinator.failed(
                state_key,
                payload_hash,
                error,
                remote_started=entry is not None and entry.status in {PublishStatus.PUBLISHING, PublishStatus.UNCERTAIN},
            )
        if batch.errors:
            failed_target, error = next(iter(batch.errors.items()))
            label = "PC日报" if failed_target == "pc" else "主日报"
            entry = publish_store.entry(f"feishu_{failed_target}", hashes[failed_target])
            if entry is not None and entry.status is PublishStatus.UNCERTAIN:
                raise PublishUncertainError(f"飞书{label}发送结果待确认，已阻止自动重发: {error}") from error
            raise ReportError(f"飞书{label}推送失败: {error}") from error

    if should_push_wecom_bot(config, args):
        for target in resolve_wecom_auto_targets(config):
            ...
            try:
                ...
                coordinator.completed(state_key, payload_hash, {"message_count": result.get("message_count", 0)})
            except WeComBotError as exc:
                coordinator.failed(state_key, payload_hash, exc, remote_started=True)
                raise PublishUncertainError(f"企业微信发送结果待确认({target})，已阻止自动重发: {exc}") from exc
    return urls
...
async def execute_daily_pipeline(
    ...
    async def authenticate(ctx: RunContext) -> Any:
        return await authenticate_daily_sources(ctx, config, args, extra_metrics_cfg, extra_auth_file, pc_web_cfg)

    async def collect(ctx: RunContext) -> Any:
        return await collect_daily_sources_with_auth(
            ctx,
            args,
            cookie,
            ...
            pc_web_cfg,
            timeout,
            ctx.shared["authenticate"],
        )

    async def calculate(ctx: RunContext) -> Any:
        return calculate_daily_report_data(config, ctx.shared["collect"])

    async def render(ctx: RunContext) -> Any:
...
def _prepare_runtime_paths(args: argparse.Namespace) -> None:
    ...
    if migration.changed:
        logging.info("V1.5 runtime configuration migrated: %s", migration.message)
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    try:
        ...
        if isinstance(exc, (PublishUncertainError, UncertainPublishError)):
        ...
        emit_event(
            ...
            details={
                ...
                "failure_source": exc.source if isinstance(exc, SourceError) else "",
...
def run_with_args(args: argparse.Namespace) -> None:  # noqa: C901 - compatibility CLI routing
    ...
    if args.push_report_file is not None:
        ...
        publish_coordinator = PublishCoordinator(publish_store, force=args.force_publish)
        publish_hash = build_publish_hash(report_file, [*chart_image_paths.values(), *payment_images.values()])
        completed = publish_coordinator.completed_result("feishu_main", publish_hash)
        if completed is not None:
        ...
        try:
            ...
            callbacks = publish_coordinator.callbacks("feishu_main", publish_hash)
            feishu_result = publish_report_to_feishu_doc(
                ...
                chart_image_paths=chart_image_paths,
                payment_images=payment_images,
                on_publish_started=callbacks.started,
                on_document_created=callbacks.remote_created,
            )
            publish_coordinator.completed("feishu_main", publish_hash, feishu_result)
            ...
            entry = publish_store.entry("feishu_main", publish_hash)
            publish_coordinator.failed(
                "feishu_main",
                publish_hash,
                exc,
                remote_started=entry is not None and entry.status in {PublishStatus.PUBLISHING, PublishStatus.UNCERTAIN},
            )
            if (publish_store.entry("feishu_main", publish_hash) or entry).status is PublishStatus.UNCERTAIN:
                raise PublishUncertainError(f"飞书文档发送结果待确认，已阻止自动重发: {exc}") from exc
    ...
    if args.push_pc_report_file is not None:
        ...
        completed = publish_coordinator.completed_result("feishu_pc", publish_hash)
        if completed is not None:
        ...
        try:
            ...
            callbacks = publish_coordinator.callbacks("feishu_pc", publish_hash)
            feishu_result = publish_report_to_feishu_doc(
                settings=feishu_settings,
                report_text=report_text,
                report_date=report_date_for_push,
            ...
            publish_coordinator.completed("feishu_pc", publish_hash, feishu_result)
            ...
            entry = publish_store.entry("feishu_pc", publish_hash)
            publish_coordinator.failed(
                "feishu_pc",
                publish_hash,
                exc,
            ...
            if (publish_store.entry("feishu_pc", publish_hash) or entry).status is PublishStatus.UNCERTAIN:
                raise PublishUncertainError(f"PC飞书文档发送结果待确认，已阻止自动重发: {exc}") from exc
...
def cli_entrypoint() -> None:
    try:
        main()
    except (ReportError, SourceError, UncertainPublishError) as exc:
